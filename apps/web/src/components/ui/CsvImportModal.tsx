...
function autoMap(headers: string[]): Record<string, string> {
  const aliases: Record<string, string> = {
    ...
    'last name': 'lastName', lastname: 'lastName', 'last_name': 'lastName', lname: 'lastName',
      ...
      <div className="mb-6 flex items-center gap-2 text-xs text-gray-500">
        {(['upload', 'map', 'result'] as Step[]).map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold ${step === s ? 'bg-indigo-600 text-white' : i < ['upload', 'map', 'result'].indexOf(step) ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-500'}`}>
              {i + 1}
            </span>
      ...
      {step === 'upload' && (
        <div className="space-y-4">
          <div
            ...
            className={`flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed px-6 py-12 transition-colors ${dragging ? 'border-indigo-400 bg-indigo-50' : 'border-gray-300 hover:border-indigo-300 hover:bg-gray-50'}`}
          >
          ...
          <div className="rounded-lg bg-gray-50 p-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Expected CSV format</p>
            <code className="block text-xs text-gray-600">First Name,Last Name,Phone,Email,Company,Status</code>
      ...
      {step === 'map' && (
        <div className="space-y-4">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
              {headers.map((header) => (
                <div key={header} className="flex items-center gap-3">
                  <span className="w-36 truncate rounded bg-gray-100 px-2 py-1.5 text-xs font-mono text-gray-700" title={header}>{header}</span>
          ...
          {previewRows.length > 0 && (
            <div>
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Preview (first {previewRows.length} rows)</p>
      ...
      {step === 'result' && result && (
        <div className="space-y-4">
          ...
          {result.errors.length > 0 && (
            <div>
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Errors</p>
