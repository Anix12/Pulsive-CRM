...
export const list = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { applications, meta } = await service.list(req.tenantId!, req);
    sendSuccess(res, applications, 200, meta);
  } catch (err) { next(err); }
};

export const funnel = async (req: Request, res: Response, next: NextFunction) => {
  try {
    sendSuccess(res, await service.funnel(req.tenantId!));
  } catch (err) { next(err); }
};
